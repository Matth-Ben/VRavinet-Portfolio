<template>
  <div id="contact">
    <h2 class="contact--title">Contact</h2>
    <hr class="separator grey mt-1">
    <div class="row">
      <div class="col8">
        <div class="contact--info">
          <h3 class="contact--info--item">
            <a class="contact--info--item--link" href="https://goo.gl/maps/kiEZiQ9E9gjGEhDY6" target="_blank">
              4 Rue Louis Braille, 69100 Villeurbanne
            </a>
          </h3>
          <h3 class="contact--info--item">
            <a class="contact--info--item--link" href="tel:0619083980">
              06 19 08 39 80
            </a>
          </h3>
          <h3 class="contact--info--item">
            <a class="contact--info--item--link" href="mailto:victorravinet@gmail.com">
            victorravinet@gmail.com
            </a>
          </h3>
        </div>
      </div>
      <div class="col3">
        <div class="contact--rs">
          <a href="https://www.instagram.com/victorrvnt/" target="_blank"><i class="fab fa-instagram"></i></a>
          <a href="https://www.linkedin.com/in/victor-ravinet-59aba3172/" target="_blank"><i class="fab fa-linkedin-in"></i></a>
        </div>
      </div>
    </div>
    <hr class="separator grey mb-0">
  </div>
</template>

<script>
export default {
  name: 'Contact'
};
</script>
